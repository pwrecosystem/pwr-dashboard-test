...
/**
 * Categoriza un servicio por la descripción de factura.
 * Útil para desglosar Amsterdam en sub-categorías.
 */
export function categorizarServicio(descripcion) {
  const desc = (descripcion || '').toLowerCase()
  if (desc.includes('wellness') || desc.includes('masaje') || desc.includes('tensa') || desc.includes('liberacion') || desc.includes('aparatologia')) return 'Wellness'
  if (desc.includes('nutricion') || desc.includes('nutri deport')) return 'Nutrición'
  if (desc.includes('box')) return 'Box'
  if (desc.includes('evento') || desc.includes('fest')) return 'Eventos'
  if (desc.includes('yoga')) return 'Yoga'
  return 'Training'
}
